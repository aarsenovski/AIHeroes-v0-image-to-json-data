...
export async function searchAlgoliaProducts(
  ...
  try {
    ...
    const searchQuery = Array.isArray(searchTerms)
    ...
    return (
      firstResult && "hits" in firstResult ? firstResult.hits : []
    ) as AlgoliaProduct[];
  } catch (error) {
...
export async function searchAlgoliaProductsBatch(
  queries: AlgoliaSearchParams[]
): Promise<AlgoliaProduct[][]> {
  return await Promise.all(
    queries.map((query) => searchAlgoliaProducts(query))
  );
}
